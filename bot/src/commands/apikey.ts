...
export const apikeyCommand: Command = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand.setName('status').setDescription('現在のAPIキー設定を確認します')
    )
    .addSubcommand((subcommand) =>
      subcommand.setName('test').setDescription('APIキーの有効性をテストします')
...
async function handleSet(
  ...
  if (url && provider !== 'self-hosted') {
    await interaction.editReply('❌ URLはSelf-hostedプロバイダーのみ指定できます');
    return;
  }
