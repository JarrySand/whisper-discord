...
export const botConfig: BotConfig = {
  ...
  audio: {
    ...
    maxSegmentDuration: getEnvNumberOrDefault(
      "AUDIO_MAX_SEGMENT_DURATION",
      30000,
    ), // 30秒セグメント
    minSegmentDuration: getEnvNumberOrDefault(
      "AUDIO_MIN_SEGMENT_DURATION",
      1000,
    ),
    minRmsThreshold: parseFloat(
      process.env["AUDIO_MIN_RMS_THRESHOLD"] ?? "0.01",
    ), // 低エネルギーセグメント除去（コスト削減・ハルシネーション防止）
  },
  ...
  output: {
    ...
    enableDiscordPost: getEnvBooleanOrDefault(
      "OUTPUT_ENABLE_DISCORD_POST",
      true,
    ),
    ...
    discord: {
      format: getEnvOrDefault("OUTPUT_DISCORD_FORMAT", "standard") as
        | "standard"
        | "compact"
        | "embed",
      showTimestamp: getEnvBooleanOrDefault(
        "OUTPUT_DISCORD_SHOW_TIMESTAMP",
        true,
      ),
      showConfidence: getEnvBooleanOrDefault(
        "OUTPUT_DISCORD_SHOW_CONFIDENCE",
        false,
      ),
      batchMessages: getEnvBooleanOrDefault(
        "OUTPUT_DISCORD_BATCH_MESSAGES",
        true,
      ),
      batchIntervalMs: getEnvNumberOrDefault(
        "OUTPUT_DISCORD_BATCH_INTERVAL_MS",
        3000,
      ),
    },
  },
