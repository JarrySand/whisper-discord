...
describe("HallucinationFilter", () => {
  ...
  describe("繰り返しパターン検出", () => {
    ...
    test("同じ単語の繰り返しを検出する", () => {
      const result = filter.detectRepetition(
  ...
  describe("定型フレーズパターン検出", () => {
    ...
    test("「ご視聴ありがとうございました」を検出する", () => {
      expect(
        filter.detectPatternHallucination("ご視聴ありがとうございました"),
      ).toBe(true);
    });

    test("「チャンネル登録」を検出する", () => {
      expect(filter.detectPatternHallucination("チャンネル登録よろしく")).toBe(
        true,
      );
    });
    ...
    test("通常のテキストは検出しない", () => {
      expect(
        filter.detectPatternHallucination("今日の会議について話しましょう"),
      ).toBe(false);
    });
  });
