...
describe("JsonStoreService", () => {
  ...
  describe("参加者情報", () => {
    ...
    test("同じ参加者の発話数がカウントされる", async () => {
      ...
      await service.addSegment(
        createMockResult({ userId: "user-1", durationMs: 1000 }),
      );
      await service.addSegment(
        createMockResult({ userId: "user-1", durationMs: 2000 }),
      );
      await service.addSegment(
        createMockResult({ userId: "user-1", durationMs: 3000 }),
      );
  ...
  describe("統計計算", () => {
    ...
    test("平均信頼度が計算される", async () => {
      ...
      await service.addSegment(createMockResult({ confidence: 0.9 }));
      await service.addSegment(createMockResult({ confidence: 0.8 }));
      await service.addSegment(createMockResult({ confidence: 0.7 }));
  ...
  describe("ファイル保存", () => {
    test("prettyPrint=trueで整形されたJSONが保存される", async () => {
      ...
      await prettyService.startSession(
        "guild-1",
        "Test Guild",
        "channel-1",
        "general",
    ...
    test("prettyPrint=falseで圧縮されたJSONが保存される", async () => {
      ...
      await compactService.startSession(
        "guild-1",
        "Test Guild",
        "channel-1",
        "general",
