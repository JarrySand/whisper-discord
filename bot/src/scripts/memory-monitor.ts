...
function displayMemory(): void {
  ...
  const delta =
    history.length > 1 ? snapshot.rssMB - history[history.length - 2].rssMB : 0;
  const deltaStr = delta >= 0 ? `+${delta.toFixed(2)}` : delta.toFixed(2);
  const deltaColor =
    delta > 5 ? "\x1b[31m" : delta > 0 ? "\x1b[33m" : "\x1b[32m";
  ...
  if (snapshot.rssMB / MEMORY_LIMIT_MB >= CRITICAL_THRESHOLD) {
    console.log(
      "\x1b[31m⚠️  CRITICAL: メモリ使用量が危険レベルです！OOMの危険があります\x1b[0m",
    );
...
console.log(
  `║ 初期 RSS: ${formatMemMB(initial.rssMB * 1024 * 1024).padStart(8)} MB | Heap: ${formatMemMB(initial.heapUsedMB * 1024 * 1024).padStart(8)} MB              ║`,
);
console.log(
  "║                                                                              ║",
);
