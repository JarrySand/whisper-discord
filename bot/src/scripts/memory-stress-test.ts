...
function logMemory(label: string): void {
  const mem = getMemory();
  const usagePercent = (
    (mem.rss / 1024 / 1024 / MEMORY_LIMIT_MB) *
    100
  ).toFixed(1);
  console.log(
    `[${label}] Heap: ${formatMB(mem.heap)} MB | RSS: ${formatMB(mem.rss)} MB (${usagePercent}% of ${MEMORY_LIMIT_MB}MB)`,
  );
}
 ...
 */
async function simulateSingleUserSegment(
...
async function runTest() {
  ...
  console.log(
    `║ 最終 RSS: ${formatMB(final.rss).padStart(8)} MB                                                  ║`,
  );
  console.log(
    `║ 最終 Heap: ${formatMB(final.heap).padStart(7)} MB                                                  ║`,
  );
  console.log(
    `║ 使用率: ${((finalRssMB / MEMORY_LIMIT_MB) * 100).toFixed(1).padStart(5)}% of ${MEMORY_LIMIT_MB}MB                                                ║`,
  );

  if (finalRssMB < MEMORY_LIMIT_MB * 0.5) {
    console.log(
      "║ \x1b[32m✓ 512MB環境で安定動作の可能性が高い\x1b[0m                                     ║",
    );
  } else if (finalRssMB < MEMORY_LIMIT_MB * 0.7) {
    console.log(
      "║ \x1b[33m⚠ 512MB環境で動作可能だが、余裕は少ない\x1b[0m                                 ║",
    );
  } else {
    console.log(
      "║ \x1b[31m❌ 512MB環境では不安定になる可能性\x1b[0m                                       ║",
    );
  }
  console.log(
    "╚══════════════════════════════════════════════════════════════════════════════╝",
