...
最初に `/join` を実行する際、文字起こし結果を投稿するチャンネルを指定できます。

```
/join output_channel:#文字起こし
```

一度設定すると、次回以降は自動で同じチャンネルに投稿されます。
設定しない場合は、`/join` を実行したチャンネルに出力されます。

### 4. プロンプトを設定する（推奨・管理者のみ）

Whisper APIに送信するプロンプトを設定すると、認識精度が向上します。

```
/prompt set text:DAOやNFTについてWeb3の話をしています。
```

**ヒント:** 会話の文脈や専門用語を自然な文章で書くと効果的です。
...
| `/prompt` | Whisperプロンプトを設定して認識精度を向上 |
...
### /prompt
Whisper APIに送信するプロンプトを設定して、認識精度を向上させます。

```
/prompt set text:TypeScriptとReactでWeb開発の話をしています。
/prompt show
/prompt clear
```
...
### 専門用語の認識精度を上げるには
`/prompt set` でコミュニティに合った文脈を設定すると、認識精度が向上します。
...
### Q: 認識精度が低い
A: マイクの音量を確認してください。また、`/prompt set` で会話の文脈を設定すると精度が上がります。
