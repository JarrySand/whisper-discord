...
| 項目 | コスト | 備考 |
|------|--------|------|
| **Bot サーバー** | 月額 $7〜 | Render 512MB プランで十分動作 |
| **Whisper API (Groq)** | 約 $0.10〜0.20/時間 | 1時間のミーティングあたり |
| **Whisper API (Local)** | 無料 | 自前PCで処理（電気代のみ） |
| **Whisper API (OpenAI)** | 約 $0.36/時間 | $0.006/分 |
...
| シナリオ | 計算 | コスト |
|---------|------|--------|
| 1時間の音声を全部送信 | $0.0001 × 3600秒 | $0.36 (約54円) |
| 実際の発話 50% の場合 | $0.0001 × 1800秒 | $0.18 (約27円) |
| 無音除去 + フィルター後 (30-40%) | $0.0001 × 1200秒 | $0.12 (約18円) |

> 💡 本システムでは **無音区間の自動除去**、**低音量セグメントのスキップ**、**最小セグメント長フィルター** が効いているため、実際のコストは抑えられます。

### 月額コスト例

| 使用パターン | 月額目安 |
|-------------|---------|
| 週1回・1時間のミーティング | **$7.80**（Bot $7 + Groq $0.80） |
| 週3回・1時間のミーティング | **$9.40**（Bot $7 + Groq $2.40） |
| 毎日1時間のミーティング | **$13**（Bot $7 + Groq $6） |
